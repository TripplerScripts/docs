...
lib.promise.on('createMultiplePeds', function(peds, defaultSettings)
...
### Examples
```lua filename="client.lua"
lib.promise.on('createSinglePed', function(settings)
  local fallback<const> = lib.promise.await({ true }, 'getDefaultSettings', settings)
