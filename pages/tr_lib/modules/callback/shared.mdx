...
registerCallback('getData', function(source, playerId)
    return {id = playerId, name = 'John'}
end)
```

```lua filename="client/main.lua"
registerCallback('getHealth', function(senderId)
...
```lua filename="wrapper/callback.lua"
function awaitServerCallback(name, timeout, ...)
  return exports.tr_lib:callback().awaitServer(name, timeout, ...)
end
```

```lua filename="client/main.lua"
local result = awaitServerCallback('buyItem', 5000, 'bread', 2)
...
```lua filename="server/main.lua"
local tax = awaitServerCallback('calculateTax', 5000, 1000)
print('Tax:', tax)
```
...
```lua filename="wrapper/callback.lua"
function awaitClientCallback(name, playerId, timeout, ...)
  return exports.tr_lib:callback().awaitClient(name, playerId, timeout, ...)
end
```

```lua filename="server/main.lua"
RegisterNetEvent('someEvent', function()
    local info = awaitClientCallback('getPlayerInfo', source, 5000)
    if info then
...
local response = awaitClientCallback('tradeRequest', targetPlayerId, 5000, 'Diamond')
